...
class GdThumb extends ThumbBase
{
	...
	/** 
	 * 自行添加缩微图处理，将图片等比例缩小，图片大小为设置大小，需填充部分填充颜色
	 * @param unknown $width
	 * @param unknown $height
	 * @throws InvalidArgumentException
	 * @return GdThumb
	 */
	public function dbshopResize($width, $height)
	{
	    // make sure our arguments are valid
	    if ((!is_numeric($width) || $width  == 0) and (!is_numeric($height) || $height == 0))
	    {
	        throw new InvalidArgumentException('$height or $width must be numeric and greater than zero');
	    }

	    $scaleOrg = $this->currentDimensions['width'] / $this->currentDimensions['height'];

	    if($width == 0)
	    {
	        $width = $height * $scaleOrg;
	    }
	    if($height == 0)
	    {
	        $height = $width / $scaleOrg;
	    }
	    // create the working image
	    if (function_exists('imagecreatetruecolor'))
	    {
	    ...
	    $clr = imagecolorallocate($this->workingImage, $red, $green, $blue);
	    imagefilledrectangle($this->workingImage, 0, 0, $width, $height, $clr);
	    
	    if($this->currentDimensions['width'] / $width > $this->currentDimensions['height'] / $height)
	    {
	        $lessenWidth  = $width;
	        $lessenHeight = $width / $scaleOrg;
	    }
	    else 
	    {
	        $lessenWidth  = $height * $scaleOrg;
	        $lessenHeight = $height;
	    }
	    
	    $dstX = ($width  - $lessenWidth)  / 2;
	    $dstY = ($height - $lessenHeight) / 2;
	    
		    if(function_exists('imagecopyresampled'))
	    {
	        imagecopyresampled($this->workingImage, $this->oldImage, $dstX, $dstY, 0, 0, $lessenWidth, $lessenHeight, $this->currentDimensions['width'], $this->currentDimensions['height']);
	    }
	    else 
	    {
	        imagecopyresized($this->workingImage, $this->oldImage, $dstX, $dstY, 0, 0, $lessenWidth, $lessenHeight, $this->currentDimensions['width'], $this->currentDimensions['height']);
	    }
